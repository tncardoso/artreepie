...
// Scope functions are not thread safe and should be used with care.
type Scope struct {
	parent *Scope
	fset   *ast.FileSet
...
func NewScope(fset *ast.FileSet) *Scope {
	...
	return &Scope{fset: fset, vars: vars, running: true}
}
...
func (s *Scope) Branch() *Scope {
	return &Scope{parent: s, fset: s.fset, running: s.running}
}
...
// Kill ongoing evaluation. If this happend during an evaluationg then
// 'Aborted' error is returned. After an abortion the scope cannot
...
func (s *Scope) Eval(node ast.Node) (value interface{}, err error) {
    if !s.running {
		...
		return nil, s.errorAt(node, fmt.Errorf("Aborted"))
    }
